...
public class VSManager : NetworkBehaviour {
    ...
    public void MatchMode()
    {
        ...
        if (HReady && GReady && isServer && isLocalPlayer && !BothReady)
        {
    ...
    public void SongSlect()
    {
        ...
        if (isLocalPlayer)
        {
            ...
            Debug.Log("Round: " + VSSlectManager.Round + "CanSlect?: " + VSSlectManager.CanSlect + "NextSong: " + VSSlectManager.ID + "SongID: " + nowID);
        }
        if (!isLocalPlayer)
        {
            if (VSSlectManager.Round == 1)
        ...
        if (HReady && GReady && !BothReady && Host)
        {
    ...
    public void Result()
    {
        ...
        if (GReady && HReady && !BothReady && Host)
        {
            BothReady = true;
            GReady = false;
            HReady = false;
            PlayerStandBy = false;
            HostStandBy = false;
            if(VSSlectManager.Round == 3 || HWin == 2 || GWin == 2)
            {
                VSResult.StartTotalResult = true;
            }
            else
            {
                VSSlectManager.Round++;
                Invoke("GotoSongSlect", 3);
            }
        }
