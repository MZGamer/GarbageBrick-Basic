...
public class GoBack : MonoBehaviour {
    ...
    public int count;
    public GameObject Result, Total, ThanksPlay, PlayerProfile,PlayerShop;
    ...
    public void Start()
    {
        ...
        if (Gate.Login)
        {
            scale = (float)Gate.NowAccount.exp / (float)(Gate.NowAccount.level * 1017);
            Name.text = Gate.NowAccount.Name;
            LV.text = Gate.NowAccount.level.ToString();
            exp.text = Gate.NowAccount.exp.ToString() + "/" + (Gate.NowAccount.level * 1017) + " (" + scale * 100 + "%)";
    ...
    private void Update()
    {
        ...
        if (StartUpdate)
        {
            ...
            if (Gate.NowAccount.exp >= Gate.NowAccount.level * 517)
            {
                TotalScore -= Gate.NowAccount.level * 517;
            ...
            Gate.NowAccount.cash = previousCash + (int)((NowScore / 150) * progress);


                scale = (float)Gate.NowAccount.exp / (float)(Gate.NowAccount.level * 517);
            if (scale < 0)
                scale = 0;
                Name.text = Gate.NowAccount.Name;
                LV.text = Gate.NowAccount.level.ToString();
                exp.text = Gate.NowAccount.exp.ToString() + "/" + (Gate.NowAccount.level * 517) + " (" + scale * 100 + "%)";
